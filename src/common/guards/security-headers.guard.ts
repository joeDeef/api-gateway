...
export class SecurityHeadersGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const expectedApiKey = this.configService.get<string>('FRONTEND_INTERNAL_API_KEY');
    ...
    // 3. Validación de JWT (Capa de Autenticidad - RNF02)
    if (!token) {
      this.logger.warn(
    ...
    try {
      const base64Key = this.configService.get<string>('FRONTEND_PUBLIC_KEY_BASE64');

      if (!base64Key) {
        this.logger.fatal('CONFIG ERROR: FRONTEND_PUBLIC_KEY_BASE64 no definida en el entorno.');
